...
class EndoNeRF_Dataset(object):
    ...
    def format_infos(self, split):
        if self._camera_info_cache is None:
            self._camera_info_cache = self._build_camera_infos()
        ...
        return [self._camera_info_cache[idx] for idx in idxs]

    def _build_camera_infos(self):
        cameras = {}
        depths = []
        depths_tensor = []
        depths_cnt = []
        ...
        for idx in tqdm(range(len(self.image_paths))):
            ...
            depth_tmp = torch.from_numpy(depth_tmp.astype(np.float32)).unsqueeze(0).unsqueeze(1).float()
            depth_cnt = torch.from_numpy(depth_cnt.astype(np.float32)).unsqueeze(0).unsqueeze(1).float()
            ...
            color_cnt = torch.from_numpy(color_cnt.astype(np.float32)).unsqueeze(0).unsqueeze(1).float()
        ...
        depth_kernel = torch.ones(1, 1, 251, 251, device=depth_tmp_tensor.device)
        with torch.no_grad():
            depth_tmp_tensor = F.conv2d(depth_tmp_tensor, weight=depth_kernel, stride=(1,1), padding='same')
            ones_tmp_tensor = F.conv2d(depth_cnt_tensor, weight=depth_kernel, stride=(1,1), padding='same')

        color_tmp_tensor = torch.cat(colors_tensor, dim=0).cuda()
        color_cnt_tensor = torch.cat(colors_cnt, dim=0).cuda()
        color_kernel = torch.ones(3, 1, 251, 251, device=color_tmp_tensor.device)
        with torch.no_grad():
            color_tmp_tensor = F.conv2d(color_tmp_tensor, weight=color_kernel, stride=(1, 1), padding='same', groups=3)
        color_ones_tmp = F.conv2d(color_cnt_tensor, weight=depth_kernel, stride=(1, 1), padding='same')
        ...
        for idx in range(len(depths)):
            ...
            time = self.image_times[original_idx]
            R, T = self.image_poses[original_idx]
            ...
            image_name = f"{original_idx}"

            cameras[original_idx] = CameraInfo(uid=original_idx, R=R, T=T, FovY=FovY, FovX=FovX, image=image, depth=depth, mask=mask,
    ...
    def get_sparse_pts_v2(self, sample=True):
        ...
        pts, colors = self.search_pts_colors(pts, colors, mask, c2w, debug=False)
    ...
    def search_pts_colors(self, ref_pts, ref_color, ref_mask, ref_c2w, debug=False):
